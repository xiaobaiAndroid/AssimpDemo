...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER void rq_decompose(mat<C, R, T, Q> const& in, mat<(C < R ? C : R), R, T, Q>& r, mat<C, (C < R ? C : R), T, Q>& q)
	{
		...
		// The RQ decomposition transforms a renderer.matrix A into the product of an upper triangular renderer.matrix R (also known as right-triangular) and an orthogonal renderer.matrix Q. The only difference from QR decomposition is the order of these matrices.
